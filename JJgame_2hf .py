...
def act_pokes(act_data,my_left_cards):
    ...
    def single_click(x,y):
        pg.moveTo(float(x)*1440+200,float(y)*990,0.3)
        pg.click()

    def drag_rec(x1,y1,x2,y2):
        pg.moveTo(float(x1)*1440+200,float(y1)*990,0.3)
        pg.dragTo(float(x2)*1440+200,float(y2)*990,0.5, button='left')
    # print(my_left_cards)
    if act_data == ['PASS']:
        ...
        if act_data[1] == 'solo':           #出单
            ...
            for i in my_left_cards:
                if i[0] == act_data[0][-1]:
                    x2 = float(i[1][0])-float(i[1][2])          #float(i[1][0])-float(i[1][2])
                    y2 = i[1][1]
            drag_rec(x1,y1,x2,y2)
...
def get_record(player_mark,record,forward_player_cards,back_player_cards,my_cards):
    ...
    # if is_over:
        ...
        #     else:
        #         if record[-1][0] == forward_player and record[-1][1] != forward_player_cards:
        ...
        #     else:
        #         if record[-1][0] == back_player and record[-1][1] != back_player_cards:
        ...
        #     else:
        #         if record[-1][0] == my_player and record[-1][1] != my_cards:
        #             record.append((back_player,['PASS']))
        #             record.append((forward_player,['PASS']))
...
def socket_client(game_num):
    while True:
        ADDR =('10.21.8.79',9999)
        reg_sock = socket.socket()
        ADDR1 =('10.21.8.67',8888)
        ...
        while True:
            # try:
                pic_path = './tmp/'
                record = get_img(i,record,win_num)
                if record == []:
                    player_mark = []
                if len(os.listdir(pic_path)) > 0:
                    ...
                    if os.path.isfile(filepath):
                        ...
                        if res == 'file send over!':
                            ...
                            my_cards_num = [x[0] for x in my_left_cards]
                            if (len(my_cards_num) == 17 and last_cards_num < 17) or (len(my_cards_num) == 20 and last_cards_num < 20):
                                record = []
                            last_cards_num = len(my_cards_num)
                            # print(record)
                            if record != [] and act_flag == True:
                                ...
                                if record[-1][0] == back_player:
                                    ...
                                    record.append((back_player,['PASS']))
                                    record.append((forward_player,['PASS']))
                            if act_flag == True:    # or len(my_left_cards) == 17 or len(my_left_cards) == 20:
                                # print(len(my_left_cards))
                                act_data = strategy_socket(my_left_cards,record,strategy_sock)
                                if act_data != '':
                                    act_data = eval(act_data)
                                    print(act_data)
                                    act_pokes(act_data,my_left_cards)
                                if act_data != ['PASS'] and act_data != '':
                                    if act_data[1] in ['bomb','rocket']:
                                        if record[-1][0] == back_player:
                                            record.append((forward_player,['PASS']))
                ...
                # if record == []:
                #     # game_num += 1
                #     print('当前游戏总局数：%s  胜率局数：%s' %(game_num,win_num))
                i += 1
            # except:
            #     reg_sock.close()
            #     strategy_sock.close()
...
def click_but():
    ...
    while True:
        if pg.pixelMatchesColor(1400, 670, (219, 80, 21), tolerance=10):   #有不叫按钮
            ...
            pass
            # pg.moveTo(900,500,duration=0.6)
            # pg.click()
...
# while True:
#     record,is_over = get_img(i,record,is_over,win_num)
#     i += 1
