...
class Card(object):
    ...
    @staticmethod
    def new(string):   #将每一张牌映射到唯一的数值 6h----35
    ...
    @staticmethod
    def card_ints_from_string(cards_str):   #将多张牌映射为数值列表 '6h-5c'---[35, 130]
    ...
    @staticmethod
    def int_to_str(card_int):       #将单一数值映射到牌 35----6h
        rank_int = Card.get_rank_int(card_int)
        suit_int = Card.get_suit_int(card_int)
        return Card.STR_RANKS[rank_int] + Card.INT_SUIT_TO_CHAR_SUIT[suit_int]

    @staticmethod
    def rank_int_to_str(card_int):     #将数值映射到无花色牌  35---6
        rank_int = Card.get_rank_int(card_int)
        return Card.STR_RANKS[rank_int]

    @staticmethod
    def cards_without_suit(card_ints):  #将列表映射为无花色牌 [0,5,14]---3-8-CJ
        no_suit_cards = [Card.rank_int_to_str(ci) for ci in card_ints]
        return '-'.join(no_suit_cards)

    @staticmethod
    def sort_cards_by_rank_int(card_ints):      #整数列表排序 [9,5,12]----[12, 9, 5]
        def cmp_card(x, y):
            return Card.get_rank_int(x) - Card.get_rank_int(y)
        
        return sorted(card_ints, key=cmp_to_key(cmp_card), reverse=False)
        

    @staticmethod
    def card_rank_to_real_card(card):       #将无花色牌变为有花色牌
    ...
    @staticmethod
    def get_rank_int(card_int):         #获取牌大小
        return (card_int) & 0xF

    @staticmethod
    def get_suit_int(card_int):         #获取牌花色
        return (card_int >> 4) & 0xF

    @staticmethod
    def int_to_pretty_str(card_int):        #将数值变为带花色的牌 35----6h
        """
        ...
        s = Card.INT_SUIT_TO_CHAR_SUIT[suit_int]#Card.PRETTY_SUITS[suit_int]
