...
class StradgeServer(BaseRequestHandler):
    def cardstype(self,cards):
        if cards == None or cards == ['PASS']:
            return None
        elif cards not in game.action_space:
            return None
        else:
    ...
    def get_actions_candy(self,cards):
        ...
        for i in acts:
            ...
            if act_type not in ['trio_solo','trio_pair','trio_solo_chain_2','trio_solo_chain_3','trio_solo_chain_4','trio_solo_chain_5','four_two_solo','four_two_pair','trio_pair_chain_2','trio_pair_chain_3','trio_pair_chain_4',]:
    ...
    # def new_actions_candy(self,actions_candy,act_card):
    #     if type(actions_candy) is tuple:
    #         for i in actions_candy[1]:
    #             if i == act_card:
    #                 actions_candy[1].remove(act_card)
    #                 break
    #         actions_candy = (int(actions_candy[0])-1,actions_candy[1])
    #         return actions_candy
    #     elif type(actions_candy) is list:
    #         unuse = []
    #         for x in range(len(actions_candy)):
    #             mark = False
    #             for i in actions_candy[x][1]:
    #                 if i == act_card:
    #                     actions_candy[x][1].remove(act_card)
    #                     mark = True
    #                     break
    #             if mark:
    #                 unuse.append((int(actions_candy[x][0])-1,actions_candy[x][1]))
    #         return unuse
        ...
        if act_mark[0] == players[1] and player == players[2] and (acted in [['CJ'],['BJ'],['2'],['A'],['A','A'],['2','2']] or self.cardstype(acted) not in ['solo','pair']):
            ...
            if act_mark[0] == player:           #主动出牌
                if type(actions_candy) is tuple:
                    ...
                    if (action.count('2') >= 2 and len(actions_candy[1]) > 2) or self.cardstype(action) in ['bomb','rocket']:
                ...
                action = ['PASS']
            elif act_mark[0] == 'player1' and acted not in [['BJ'],['2']] and player == 'player2' and action == ['CJ']:
                if (type(actions_candy) is tuple and len(actions_candy[1]) >= 4) or (type(actions_candy) is list and len(actions_candy[0][1])>=4):
                    action = ['PASS']
            actions.clear()
